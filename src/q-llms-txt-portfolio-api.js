...
export default async function ({ user, weight = 1.0 }) {
  ...
  const validate = async (submittedText) => {
    ...
    // Check for common date formats with year range 2000-2199
    const datePatterns = [
      /\b(20\d{2}|21\d{2})[-/]\d{1,2}[-/]\d{1,2}\b/,  // YYYY-MM-DD or YYYY/MM/DD
      /\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{1,2},? (20\d{2}|21\d{2})\b/i,  // Month DD[,] YYYY (comma optional)
